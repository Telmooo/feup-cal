...
Para adicionar novos pedidos, o utilizador terá que indicar o \textpf{local de recolha e destino dos prisioneiros}, o \textpf{número de prisioneiros}, \textpf{tipo de prisioneiros}, \textpf{peso da distância no trajeto} e o \textpf{peso do tempo no trajeto}
...
O modelo foi dividido em três iterações, sendo as primeiras duas problemas do tipo \textpf{caminho mais curto entre dois vértices}. Por outro lado, a terceira e última iteração é equiparada aos problemas: \textpf{Vehicle Routing Problem} e o \textpf{Travelling Salesman Problem}.

Foram utilizados múltiplos algoritmos no sentido de resolver estes problemas, nomeadamente \textpf{Dijkstra}, \textpf{A*}, \textpf{Held-Karp}, \textpf{Nearest Neighbour},\textpf{Genético}, \textpf{Kosaraju}, \textpf{Tarjan}.

Estes algoritmos envolvem conhecimentos em várias áreas da programação, transpondo os conceitos abordados na cadeira, tais como: \textpf{bruteforce}, \textpf{recursão}, \textpf{programação dinâmica}, \textpf{algoritmos gananciosos} ( \textpf{Dijkstra}, \textpf{A*}, \textpf{Nearest Neighbour}), \textpf{heurísticas}, vários conceitos associados a algoritmos genéticos, tais como, \textpf{mutação}, \textpf{crossover}, \textpf{seleção} e outros tópicos associados a grafos, nomeadamente, \textpf{conetividade} e  \textpf{ordem topológica}.
...
\begin{itemize}
	...
	\item \color{blue} \underline{\href{ https://en.wikipedia.org/wiki/Branch_and_bound}} \color{black}
\end{itemize}
