...
\subsection{Terceira Iteração - Recolha seletiva de prisioneiros utilizando vários veículos}
	Na terceira iteração serão considerados não só o número de veículos disponíveis, mas também o número de pedidos de transporte de prisioneiros. Nesta última iteração, um veículo pode atender vários pedidos, com diversos pontos de interesse diferentes, desde que não afete o tempo do pedido anterior e não ultrapasse a capacidade do veículo.
...
O modelo foi dividido em três iterações, sendo as primeiras duas problemas do tipo \textbf{caminho mais curto entre dois vértices}. A terceira e última iteração é equiparada aos problemas: \textbf{Vehicle Routing Problem} e o \textbf{Travelling Salesman Problem}.
